...
- **AI Integration**:
    - **Cerebras API**: Note summarization and tag suggestion (LLM).
    - **Groq Whisper API**: High-speed audio transcription.
...
- **Provider Orchestration**: 
    - **Groq (Whisper-large-v3)**: Used for high-speed, low-latency STT (Speech-to-Text).
    - **Cerebras (Llama-3.1-70b)**: Used for text summarization and semantic tag extraction.
- **Request Lifecycle**: 
...
The app follows a **Local-First** strategy.
1. Save to Hive (synchronous availability).
2. Push to Firestore (asynchronous background task).
